...
- **Python Anti-Patterns — AVOID THESE (write correct code on first pass):**
  ...
  57. **Adding validators to existing schemas can reject previously-valid data** — When adding `@field_validator` to an existing schema, trace ALL code paths that construct or submit data through it. Password complexity goes on `PasswordResetRequest`/`RegisterRequest`, NEVER on `LoginRequest` — existing users with weak passwords get 422 at login. Always ask: "Will this validator reject data that already exists in production?" If yes, the validator belongs on a different schema.
  58. **Tasks that remove or rename response fields must update downstream tests** — When removing a field from a response dict (health redaction, version info removal), immediately grep for the old field name across `app/*/tests/` and `app/tests/`, then update or remove all assertions on that field in the SAME step.
